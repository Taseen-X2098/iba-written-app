...
All acknowledged answers live in one Redis hash at `attempt:{attemptId}:drafts`; atomic `HSET` batches avoid per-question keys and `KEYS` scans. Dirty answers flush every 30 seconds, after OCR, on manual save, and when the page becomes hidden. Exam OCR also writes its recognized text from the server before responding, with a durable Postgres operation record as the finalization-safe fallback. Only unacknowledged edits are kept locally, AES-GCM encrypted with a key held in tab-scoped session storage. Failed acknowledgements stay dirty.

### 6.5 Atomic Completion and Abandonment
`finalize_exam_attempt` locks the official attempt, snapshots the latest acknowledged drafts, inserts every exam answer, assigns explicit zero grades to blanks, and marks finalization in one transaction. Before transferring an exam page image, the client makes a lightweight request that creates a database OCR lease. This closes the image-upload gap: finalization cannot retain an older answer while its replacement photo is still transferring or scanning. The last finishing OCR request stores the replacement text and triggers the same idempotent finalizer, so no extra worker is required. Duplicate calls return the completed state. Connected clients finalize their own attempts, while admins can explicitly finalize one expired attempt or all expired attempts for an exam from the admin panel.
